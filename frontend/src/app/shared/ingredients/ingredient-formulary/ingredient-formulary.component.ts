...
@Component({
  selector: 'app-ingredient-formulary',
  templateUrl: './ingredient-formulary.component.html',
  styleUrls: ['./ingredient-formulary.component.scss'],
})

export class IngredientFormularyComponent extends ModalFormSkeleton implements OnInit {
  ...
  async loadEditor() {
    if (this.modifyId) {
      try {
        ...
        this.form.patchValue({ name: this.oldData!.name, type: this.oldData!.type, picture: this.imagePreview })
